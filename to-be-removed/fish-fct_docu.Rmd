...
In order to merge all data from various FCTs into one common FCT data library, we needed to harmonise the format and data structure in each dataset. The steps that were taken are explained
...
food components were used for avoiding missing values. For example:
...
available carbohydrates were analysed, we used an iteration of the **Eq. 3.4a** to calculate ash concentration (Eq.3.4b).

3. **Beta-carotene equivalent** is used to calculate Vitamin A (both REA and RE), and it should be, when possible, calculated using the carotenoids values (Eq.7a). Despite that, sometimes all the carotenoids values were not available/reported in the FCTs. Hence, to re-calculate beta-carotene equivalent, two approach were taken. First, if Beta-carotene equivalent was reported in the FCT, it was used. Otherwise, if retinol and Vitamin A were reported, Beta-carotene equivalent was back calculated from those values using the Eq. 7b. 
...
using the **Eq.8.3a**. Additionally, when Vitamin A (RAE)
(`VITA_RAEmcg`) and Vitamin A (RAE) (`VITAmcg`) was provided,
retinol as calculated by the **Eq.8.3b**. This function
...
imputation techniques (see Section xx).
...
with the alternative analytical method for UF16, WA19 and KE18. In KE18, brackets denoting not preferred values, as for the KE18, UF16 and WA19
...
In addition, the standard fish identification was done for the N021 which is detailed in NO21 documentation. 
...
KE18, UK21, and WA19 ("tr"). These operation were performed using the function (`no_brackets_tr()`) to change trace and below the detection limit values to zero, and to remove brackets (see `functions.R`). When different characters or symbols were used, manual recode was needed. For instance, characters and symbols were used to report missing values were "nv" in DK19, "-" in JA15, and "M" in NO21 which were all transformed to `NA`. Similarly, nutrients present in significant quantities (not trace), but without reliable information on the amount, were reported in UK21 as "N" which were converted to `NA`.


#### 3.2 & 3.3. Unit of measurement transformation and re-calculation <br>
...
Fish matching between FCTs fish entries and ICS FAOSTAT fish category, alpha-three code and ISSCAAP code was previously done in the Fishery Global NCT, 2022. Hence, we used that information to generate a fisheries data library which
contained information for each fish entry regarding ISSCAAP, ICS FAOSTAT and
...
"0807"). In order to match the KE18 food ids (`fdc_id`) in our dataset, we removed the first "0" (e.g., "807"). For the US19, we needed to change the `fdc_id` to `NDB_number`. Then, we merged `ics_code_file` (the dataset derived
...
in the fisheries NCT library with all the fishery products and its corresponding ICS
...
#### 3.4. Calculated and combined variables (standardised) <br>

Special attention was given to a selection of nutrients based on their importance to health and because fish and aquatic products are one of the main dietary sources of those nutrients, while other nutrients were finally excluded due to the limited data availability. 
...
Sum of proximate (=sum of water + protein + fat + available carbohydrates +
...
Carbohydrates by difference were calculated using the function `CHOAVLDFg_std_creator()` that was developed based on the Eq.6a. In NO21, the value of ash (g/100g EP) used were calculated. In addition, we found `r round(sum(str_detect(fao_fish_fct$comment, "CHOAVLDFg_std assumed zero"), na.rm = T)/length(fao_fish_fct$CHOAVLDFg_std)*100)`% of carbohydrates by difference values were negatives which were converted to zero ("assumed zero") and added a comment "CHOAVLDFg_std assumed zero" for documentation. 
...
In addition, when we compared the assumed zero and other fractions, particularly those that are analytical values, we can see that there is a small underestimation of the available carbohydrates when using the "by difference" and assumed zero, as we can see that there were 95 unique food entries with carbohydrates > than zero, in those assumed zero. 
...
We combined three Tagnames into `FAT_g_standardised` as reported in section 3.4. We checked the contribution from each of the fractions Table x(tab:fat).
...
fao_fish_fct %>% select(starts_with("VITB6")) %>% mutate_all(as.numeric) %>% 
  summarise_all(., ~sum(!is.na(.))/sum(!is.na(fao_fish_fct$VITB6_mg_standardised))*100) %>% round(.,2) %>% knitr::kable(., caption = "The table present the percentage of each vitamin B6 tagname contribution to the total.")
...
fao_fish_fct %>% select(starts_with("THIA")) %>% mutate_all(as.numeric) %>% 
  summarise_all(., ~sum(!is.na(.))/sum(!is.na(fao_fish_fct$THIAmg_std))*100) %>% round(.,2) %>% knitr::kable(., caption = "The table present the percentage of each thiamine tagname contribution to the total.")
...
```{r fig-1, fig.cap= "Percentage of missing values per component and food composition table.", echo=FALSE}
...
```{r fig-2, fig-2, fig.cap= "Coverage: Percentage of reported values per component.", message=FALSE}
...
**Table 5: Count of values per component and SUA fish category**


```{r list-missing, echo=F}
...
There were only four ICS FAOSTAT categories that needed data impututation. The rationale is described below for each one:
...
`r names(fao_fish_fct)[c(13:33, 39:40, 43:49, 58:63, 69:72, 74:75,  83:93, 101:102, 104:113)]`
...
Fibre concentration of fish and other aquatic animals were mostly zero `r length(fao_fish_fct$FIBTGg[fao_fish_fct$FIBTGg==0])/dim(fao_fish_fct)[1]*100`, then smaller proportion were >0 (`r  length(fao_fish_fct$FIBTGg[fao_fish_fct$FIBTGg>0])`)
and/or `NA` (`r sum(is.na(fao_fish_fct$FIBTGg))`) when raw whereas preparations and canned were below 1g per
100g. Then, aquatic plants were on average higher than 1g up to 41g per
100g for aquatic plants preparations.

Current mean of FIBTGg was `r mean(fao_fish_fct$FIBTGg, na.rm = TRUE)`, SD `r sd(fao_fish_fct$FIBTGg, na.rm = TRUE)`.
...
"zero" as assumed values. These values were not imported/imputed in our dataset,
...
Of the twelve FCTs, 10 did not report `ALCg`, hence in contrast with fibre, most of the values were `NA` (`r sum(is.na(fao_fish_fct$ALCg))/dim(fao_fish_fct)[1]*100`, n=`r sum(is.na(fao_fish_fct$ALCg))`.  
Of the three that did report it, the mean value was `r mean(fao_fish_fct$ALCg, na.rm = TRUE)`. Hence, we could assume alcohol was zero for fish and products.

    
###### 5.2.3. Carbohydates and other re-calculated variables

Five FCTs/FCDBs did not report CHOAVLDFg (`r fao_fish_fct %>% filter(is.na(CHOAVLDFg)) %>% distinct(source_fct)`). Of those, two reported CHOCDFg instead (`r fao_fish_fct %>% filter(is.na(CHOAVLDFg), !is.na(CHOCDFg))%>% distinct(source_fct)`), and other two reported CHOAVLg (`rfao_fish_fct %>% filter(is.na(CHOAVLDFg), !is.na(CHOAVLg)) %>% count(source_fct)`). In addition, IN17 did not reported content of carbohydrated and fibre for fish and
    products. 
...
exactly the same. Of the other 60% of the values variation occurred from
 max-retinol-mcg to min-retinol-mcg. However, taking in
...
mean is YYmcg in comparison with
the
XXmcg.
...
optimal. Therefore, including more fishery entries with minimal missing values should be sought in the future.


## Edible portion 

See Edible portion (section 3.4.4)
...
#### Data imputation and other data managamenet techniques and their impact in summary statistics

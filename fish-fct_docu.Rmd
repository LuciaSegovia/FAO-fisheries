...
the values, overall all values seemed plausible. For example, one of the
...
`FAT_g_standardised` as reported in section 4.4.
...
in Se of the category, it would only increase the mean. Again from the
"Aquatic mammals, meat", we only have 3 values out of 15 values, which
makes the mean quite high, although we can assess if that mean is high
or is in line with the overall category mean due to the lack of data
availability.

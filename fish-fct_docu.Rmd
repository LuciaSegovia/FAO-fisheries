...
```{r fig-2, fig-2, fig.cap= "Figure 2: Coverage: Percentage of reported values per component.", message=FALSE}
...
`r fao_fish_fct %>% filter(!is.na(FIBTGg)) %>% pull(FIBTGg) %>% as.numeric() %>% mean(., na.rm = T)`.

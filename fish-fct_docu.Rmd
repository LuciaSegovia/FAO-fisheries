...
| PROCNTg                | Protein in g per 100g of EP, as reported in the original FCT and assumed to be calculated from nitrogen (NTg) content                                                                                                                                                                                                                                       | Compiled          |
| NTg                    | Nitrogen, total                                                                                                                                                                                                                                                                                                                                             | Compiled          |
| XN                     | Nitrogen conversion factor                                                                                                                                                                                                                                                                                                                                  | Compiled          |
| FATg                   | Total fat content in g per 100g of EP. Sum of triglycerides, phospholipids, sterols and related compounds obtained by a mixed solvent extraction                                                                                                                                                                                                            | Compiled          |
| FAT_g                  | fat content unknown method of calculation in g per 100g of EP                                                                                                                                                                                                                                                                                               | Compiled          |
| FATCEg                 | Fat content measured using continuous extraction (e.g., Soxhlet method) expressed in g per 100g of EP                                                                                                                                                                                                                                                       | Compiled          |
...
| CHOAVLDFg              | Available carbohydrates calculated by difference (E.g., 100 - (weight in grams of water + protein + fat + ash + fibre + alcohol in 100 g of food)) imputed from the original source                                                                                                                                                                         | Compiled          |
| CHOAVLg                | Available carbohydrates calculated by weight in g per 100g of EP                                                                                                                                                                                                                                                                                            | Compiled          |
| CHOCSMg                | Carbohydrates, total; calculated by summation. This values is the sum of the sugars, starches, oligosaccharide, and dietary fibre in g per 100g of EP                                                                                                                                                                                                       | Compiled          |
| CHOAVLMg               | Carbohydrates, available in monosaccharide equivalent. This value includes the free sugars plus dextrins, starch, and glycogen in g per 100g of EP                                                                                                                                                                                                          | Compiled          |
| CHOAVLDFg_std          | Available carbohydrates calculated by difference (Eq. 3.6).                                                                                                                                                                                                                                                                                                 | Calculated        |
| FIBTGg                 | Total dietary fibre by AOAC Prosky method expressed in g per 100g of EP                                                                                                                                                                                                                                                                                     | Compiled          |
| NSPg                   | Non-starch polysaccharide, (Englyst fibre) expressed in g per 100g of EP                                                                                                                                                                                                                                                                                    | Compiled          |
| FIBCg                  | Fibre, crude in g per 100g of EP                                                                                                                                                                                                                                                                                                                            | Compiled          |
...
Eg. Converting alcohol from weight in volume (w/v) to weight in mass.
...
Some variables are calculated from other variables, hence they need to
be recalculated to account for imputed values or when calculating mean
concentration. Some of those are: Sum of proximate, Energy (kcal, kJ),or
Carbohydrates available, by difference among others. Other variable that
is often re-calculated is edible portion from refuse. A special case was ash values which are needed to calculate Available carbohydrate by difference. In the NO21, ash values were not reported, however, because available carbohydates were analysed, we used the an iteration of the **Eq. 3.4a** to calculate ash concentration (Eq.3.4b).


**Eq.3.1** Edible portion = 100-Refuse(%)/100

**Eq.3.2** Edible portion = 1-Refuse

**Eq.3.3** Sum of Proximate (g/100g) = Sum of water + protein + fat +
available carbohydrates + dietary fibre + alcohol + ash

**Eq.3.4a** Carbohydrates available by difference (g/100g) = 100 - (sum
of water + protein + fat + ash + alcohol + dietary fibre)

**Eq.3.4b** Ash by difference (g/100g) = 100 - (sum of water + protein +
fat + *available carbohydate* + alcohol + dietary fibre)

**Eq.3.5a.** Energy (kJ/100g):

**Eq.3.5b.** Energy (kcal/100g):

**Eq.3.6.** Vitamin A, retinol eq. (RE) (mcg/100g) = Retinol
(mcg/100g) + 1/6\* Beta-Carotene eq. (mcg/100g)

**Eq.3.7.** Vitamin A, retinol activity eq. (RAE) (mcg/100g): Retinol
(mcg/100g) + 1/12\* Beta-Carotene eq. (mcg/100g)

**Eq.3.8.**. Beta-Carotene eq. (mcg/100g) = 1\*Beta-carotene
...
There were cases were imputation or back calculation from one or more
food components were used to complete missing values. For example:

1. Retinol: For instance, when a food entry was missing the retinol
    value (`RETOLmcg`) but it reported Vitamin A (RAE) (`VITA_RAEmcg`)
    and Beta-Carotene eq. (`CARBEQmcg`), The retinol was calculated
    using the [equation XX](LINK). Additionally, when Vitamin A (RAE)
    (`VITA_RAEmcg`) and Vitamin A (RAE) (`VITAmcg`) was provided,
    retinol as calculated by the [equation XX](LINK). This function
...
function for the weighted sum of the listed variables.

**Imputing values**: For instance, `RETOLmcg_Recalculator()` was used to
impute retinol values by back-calculation from Vitamin A (RAE) and
Beta-Carotene eq.

Here's the list of the standardised variables as reported in the
Fisheries Global NCT, 2022 (Feb., 2022), of which all except food folate
(FOLDFEmcg) were reported and functions developed to generate them in
...
data file (data library), filter the fish entries and merged with the information
already provided by the FAO team regarding each fish entry ISSCAAP, ICS
FAOSTAT and alpha-three codes.

For this particular case the steps 3.4 and 3.5 were performed after
...
We checked the number of missing values per food component using data visualisation. We used the *package naniar* for visualising missing values by ICS SUA fishery category and by FCT. This allowed us to identify any typos or mistakes when renaming the food components, and also to identify potential food components that could be excluded due to the high number of missing values. Similarly the need for using imputation techniques (see section 3.5). 


#### 4.2. Checking quality of the values <br>

We used histograms and boxplot to identify extreme values by plotting the different components by fish type and fish preparation. These two variables were created for data visalisation purposes and did not make it to the final dataset: Fisheries Global NCT (Oct., 2022). 
...
After all the values, particularly the mean values per ICS SUA category, were evaluated the dataset was formatted to be exported into a human-friendly format (e.g. Excel).

First, a summary row was generated for each ICA SUA category which contained the final mean value for each food component of interest. Additionally, some food components were recalculated. 
...
5) Vitamin A RAE (mcg) -> If retinol (RETOLmcg) and beta-carotene eq. (CARTBEQmcg) are not present, it's not calculated. Formula = [RETOL + CARTBEQmcg/12]
6) Vitamin A RA (mcg) -> If retinol (RETOLmcg) and beta-carotene eq. (CARTBEQmcg) are not present, it's not calculated. Formula = [RETOL + CARTBEQmcg/6]

Note: For all those variables in the empty row below the summary values print ("calc."). 

### Nutrient to be assumed at the summary row:

1) Alcohol. It was assumed zero. Alcohol should be zero in all and printed in the empty row below the summary value "assumed". 
...
Here's the list of the standardised variables as reported in the
Fisheries Global NCT, 2022, of which we reported all except food folate
(FOLDFEmcg).
...
Of which functions were generated to re-calculate the following the
equations in the section 3.3 of this document and can be found in
"functions.R".
...
Vitamin B6 was reported in all but one FCT (KE18). Overall, values were
within plausible values. The highest values reported were for Tuna and
...
Again from the "Aquatic mammals, meat", we only have 3 values out of 15
values, which makes the mean quite high, although we can assess if that
mean is high or is in line with the overall category mean due to the
...
(Equation 1). Additionally, it was calculated at ICS (SUA) fish category
level.
...
from the carbohydrate by difference equation (XX). Full information was
...
Fibre, crude (FIBCg) could be removed, it did not added information on
any missing (FBGTg). For raw fishes, `NA` values for fibre could be
assume equal to zero, as they may not be analysed based on the
assumption that they will be zero for that food type.
...
Alcohol: We could assume zero for fisheries.
...
\*\*Calculating CHOAVLDFg_std (CHOAVLDF(g)\_standardized)\*\*

Finally, we evaluated the difference in `CHOVALDg` values between the
Fisheries Global NCT, 2022 (original) and the recalculated.
...
-   Fnally, the sixth was 1574 - Cephalopods, preparations nei, again
...
hist(fao_fish_fct$CHOAVLDFg2[fao_fish_fct$ICS.FAOSTAT.SUA.Current.Code == "1531"], main="Distribution of CHO values (including CHOAVLDFg and CHOAVLg) for pelagic fish, cured",
...
`FAT_g_standardised` as reported in section 4.4. We checked the
contribution from each of the fractions.
...
##### Imputed from other food entry

**Namg** **VITAmcg**: Imputed/ Re-calculated from Vitamin A (IU) which
was imputed.

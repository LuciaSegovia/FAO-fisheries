...
relevant for fish. The initial list of food component that we evaluated
...
\|8. \| Vitamin D (including D3, D2 and hidroxi forms) \| Excluded \|
here were not enough data (\Section ) \|
...
the Fisheries Global NCT, two were excluded: uPulses FCT and the
Composition of foods integrated dataset (CoFID). The later was excluded
because it did not improve the data gaps, however it was process and it
...
Eicosapentaenoic acid (EPA) (See \Table) in the 11 FCTs. Some data gaps
...
In \table , we can find information on the final list of FCDBs
included.

\Table . List of all the FCTs included, and description of their access,
...
is often re-calculated is edible portion from refuse. There were special cases were back calculation from one or more
...
available carbohydates were analysed, we used the an iteration of the
...
**Eq.4.1** Edible portion = 100-Refuse (%)/100

**Eq.4.2** Edible portion = 1-Refuse

**Eq.5** Sum of Proximate (g/100g) = Sum of water + protein + fat +
...
**Eq.7.1** Energy (kJ/100g EP): sum of protein (g/100g EP)\*17 + available carbohydate (g/100g EP)\*17 + fat(g/100g EP)\*37 +  alcohol (g/100g EP)\*29 + dietary fibre (g/100g EP)\*8

**Eq.7.2** Energy (kcal/100g EP):  sum of protein (g/100g EP)\*4 + available carbohydate (g/100g EP)\*4 + fat(g/100g EP)\*9 +  alcohol (g/100g EP)\*7 + dietary fibre (g/100g EP)\*2

**Eq.8.1.**. Beta-Carotene eq. (mcg/100g) = 1\*Beta-carotene
...
**Eq.8.3** Vitamin A, retinol activity eq. (RAE) (mcg/100g EP) = Retinol (mcg/100g EP) + 1/12\* Beta-Carotene eq. (mcg/100g EP)
...
described in the renaming section and \table 2). Of the fish-relevant
...
Data imputation was avoided whenever it was possible (i.e., when mean value for each ICS SUA fishery categoies were available (n=>1). In the cases when this condition was not met, a case-by-case approach was taken and it was documented here (See \Section) and in the final Fisheries Global NCT (see `comment`). 
...
Fisheries Global NCT, 2022 (Feb., 2022), of which all except food folate (FOLDFEmcg) were reported and calculated by functions developed to generate them in
...
#### 3.6. Producing the harmonised data library

Once the FCTs were loaded and harmonised, we combined them into a single
data file (data library), filter the fish entries and merged with the
...
visualisation. We used the *package naniar*(@ref-naniar) for visualising missing
...
In US19, Tryptophan was reported in g/100g EP which was converted to mg/100g. 
...
Carbohydrates by difference were calculated using the Eq.6a. This was possible in all the FCTs but NO21 which did not provided the ash concentration (more information on NO21 documentation). 

Then, we evaluated the difference in `CHOVALDg` values between the
...
#### 3.6. Merging all the data together <br>

Once the FCTs were loaded and harmonised, we combined them into a single data library. 

###### 3.6.1. Allocating the FAO codes: ISSCAAP, alpha-three, and ICS FAOSTAT.
...
###### 4.1. Data avaibility per each ICS FAOSTAT fish category

In figure 1 @fig-1, we can see that Cu and Vit. B6 was reported by all FCTs but
...
```{r fig-1, fig.cap= "Figure 1: Percentage of missing values per component and food composition table."}
...
```{r, fig-1, fig.cap= "Figure 2: Coverage: Percentage of reported values per component.", message=FALSE}
...
After we agreed on the components to be expanded, we evaluated the availability of data by ICS FAO fishery category. In \Table 4, we highlighted in red when no values were reported for a component in a
SUA category and in yellow when less than three values were reported per
SUA category.

As shown in red in table 4, four SUA fish categories had no values for

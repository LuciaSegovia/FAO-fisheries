---
title: "Results: Harmonisation of the food composition tables and databases"
format: html
editor: source
---
...
#### 2.2. Renaming variables: Food components definition and re-naming

Food component names and descriptions were examined in each FCT and renamed with the corresponding Tagnames.

Some FCTs reported the food components using the Tagnames which were checked, extracted and used to rename the food components. Tagnames were found in different formats, hence some name harmonisation was still needed.
...
In summary, all the FCTs needed some degree of food component renaming.7 FCT’s reported (to a certain extent) the use of Tagnames for food components identification, whereas 6 did not used the Tagnames. For the latter, in certain cases, documentation was not enough to identify the correct Tagname, for instance for vitamin E in NO21 which remainded   with the original name. In addition, we found some food components for which Tagnames are currently unavailable (e.g., ). 

#### 2.3. Standardisation of values

In summary, the most common transformation was from edible portion expressed in percentage to a fraction, which was performed in XX FCTs. Other measurement unit’s standardisation performed were from g to mg, for example, tryptophan (g/100g) to mg/100g or vice versa, for DHA and EPA g/100g to mg/100g.  and vitamin A (IU) to mcg/100g, for some foods, in US19, tryptophan from (g/100g of protein) to mg/100g EP in IN17. Reported missing values were standardised in 4 FCTs of the 12 FCTs, while trace and/or below limit of detection values were transformed to zero in 7 FCTs (IN17 ("<LOD"), BA13, BR11, JA15 ("Tr"), and KE18, and WA19 ("tr"). Brackets (and parenthesis) used to denote “lower quality values” were removed in 4 FCTs, such as JA15, KE18, WA19 and UF17. 


**a. Removing brackets or other special characters ("[]", "()", "\*")**

We generated a function (`no_brackets_tr()`) to change trace and below the detection limit values to zero, and to remove brackets (see `functions.R`).
...
**a. Removing parentheses and brackets**: 
...
with the alternative analytical method for UF16, WA19 and KE18. 

In KE18, brackets denoting not preferred values, as for the KE18, UF16 and WA19
reporting: "For alternative analytical method or lower quality.

Data from non-preferred Tagnames were presented only where preferred Tagnames
data was not available." (REF).When we removed the brackets, for future development accounting for the impact of the "estimated values" should be studied.

In addition, for the KE18, a star ("\*") was used to identify items
fortified with folic acid. Note that some issues on the reporting of fortification were found previously, however the data was not reviewed
in-depth because folate and related food components were outside the scope of this work. 



#### 2.4. Standardising unit of measurement <br>

Some components were reported in different measurement units as the recommended units in
...
In the IN17, fatty acids (including DHA and EPA) needed to be converted from mg/100g EP to g/100g EP, and amino acids (eg. Tryptophan) from g/100g PROT to mg/100g EP (Eq.2.3).

Similarly, in JA15, we need to change the unit of the fatty acids (DHA and EPA) from mg to g, and the edible portion was calculated from refuse (Eq.3.1).

In UK21, alcohol values were converted from g/100mL to g/100g (`ALCg_100mL` to `ALCg`) using [Eq. 1](). Moreover, in other to proceed with the conversion values of density (specific gravity) were imputed from the FAO/INFOODS Density database v.2 (@charrondiereFAOINFOODSDensity). The imputed values were recorded an applied using a separate script (`UK21_missing-values.R`) as part of the reproducibility and interoperability of the framework. 

Finally, when (substantial) changes, like those mentioned above, were
...
### 3. Merging all the data together <br>
...
#### 3.4.	Food name/ description standardisation and food matching  <br>

#### 3.6. Dealing with missing values <br>

##### 3.6.1. Imputing values <br>

In the fisheries case study, data imputation from different foods was avoided whenever it was possible, such as when values for each ICS SUA Fishery Category were available (n≠0) (See section 3.6.1 in _methods.qmd). In the instances when this condition was not met, a case-by-case approach was taken for each category and food component to avoid missing values. This was performed in a separate script (`missing.R`) which provide flexibility in the workflow, as it could be avoided if new data on the missing values are provided, and/or allowing to test the impact of adding those values (or others) to the final datasets. 

As reported previously, missing values were evaluated only for each ICS SUA Fishery Category, which reported no missing values with the exception of Se concentration in four categories: “Cephalopods, cured”, “Aquatic animals nei, cured”, “Aquatic animals nei, preparations nei”, "Crustaceans, cured" and "Cephalopods, canned”, which represented the 0.2% of Se values (n=11). For “cephalopods, canned”, we used fish entries from a similar category, following the approach used for other categories, such as frozen fish and explained before. For the other three categories, that approach was not possible due to the nature of the foods, hence imputation was used instead. The details of the fish entries values used, and the operations performed (e.g., water adjustment) for each imputed value are found in the documentation, and in the global fisheries NCT.


Moreover, 35% of the beta-carotene eq. values were imputed from either Beta-carotene (n= 1,422) or Beta-carotene eq. (n=297), furthermore, in 38% entries (n=1,849) the calculation of the carbohydrates available (by difference) yielded negative values which were assumed zero. Likewise, alcohol and fibre were assumed zero when used for recalculating values when those values were not reported (i.e., 3590 and 440 missing values respectively).
...
Additionally, it was calculated at ICS (SUA) fish category
level.
...
hist(as.numeric(fao_fish_fct$SOP_std), main = "Sum of Proximate",
...
Carbohydrates by difference were calculated using the function `CHOAVLDFg_std_creator()` that was developed based on the Eq.6a. In NO21, the value of ash (g/100g EP) used were calculated. 

There were 12 food entries with `CHOAVLDFg_standardised` below -5g/100g which is the lower bound advised for quality assurance recommended in the FAO/INFOODS guidelines. These food entries were identified (see `comments`), and it could be removed during the QC as per the guidelines, and given that there were enough values for their respective SUA items. 

In addition, we found `r round(sum(str_detect(fao_fish_fct$comments, "CHOAVLDFg_standardised assumed zero"), na.rm = T)/length(fao_fish_fct$CHOAVLDFg_standardised)*100)`% of carbohydrates by difference values were negatives which were converted to zero ("assumed zero") and added a comment "CHOAVLDFg_standardised assumed zero" for documentation. 
...
The value of fibre standardised was calculated using only values from the dietary fibre (`FIBTGg`), as no other fractions provided information when dietary fibre was missing values. For example, fibre, crude (FIBCg) could not be used to complete missing values in FIBTGg, as FIBCg did not added information on any missing (FIBGTg). For more details, check the section 4.2.8.
...
missing values. Additionally, the function add the documentation (i.e., "RETOLmcg value re-calculated from VITA_RAEmcg and CARTBEQmcg" or "RETOLmcg value re-calulated from VITA_RAEmcg and VITAmcg" in the `comment`). Again, this is important to 1) identify data gaps, and 2) flagging potential issues with the data. 
...
reported in the WA19, `NIAmg` was re-calculated into a new variable and combined into the Niacin standardised variable (`NIAmg_std`) using the function:`nia_conversion_creator()` as reported in the method section 3.

##### 3.4.4. Edible portion

The edible portion is reported in two variables. One is `Edible_factor_in_FCT` which is populated with information extracted from each individual FCT. The other (`Edible coefficient to be used`) is based on a combination between the edible
portion information recorded by the FAO fisheries division and the FCT information, those decision were done for the previous Fisheries Global NCT compilation. 
However, information provided in the Fisheries Global NCT, 2022 is not
sufficient to replicate it in R.

We added the `Edible coefficient to be used` in the table at the summary
row.

Because these values were imputed from the previous version, we did not performed quality control. However, we identified a potential issue with the ICS FAOSTAT fish category of some fish. There were some fishes from JA15 (e.g. 10117) that were described as "fish, raw" and were assigned to "whole fish fresh", but after checking the "edible description" they were described as "fillet". 

In the future, we could use the information in the edible description to better identify the fishery category. 
...
fao_fish_fct %>% select(components) %>% rename_all(., ~components_longname) %>% 
  ...
  annotate(geom = "text", x=0.7, y=62.5, label="60%") +
...
reported in `missing.R`, and in the `comment` variable in the Fisheries Global NCT (Oct, 2022) and the rationale is the following section. 

###### 4.2.1 Missing values & data imputation

In this section we reported the imputed values and the
annotated scripts are complied in `missing.R`.

There were only four ICS FAOSTAT categories that needed data imputation. The rationale is described below for each one:
...
In addition, one of the main source of variability found when comparing the Fisheries
...
values. This was only study in detailed for Carbohydrates by difference, Retinol and Beta-Carotene Eq. in the future a more detail study of the imputations techniques and it's overall impact and accuracy would be recommended. 

#### 5. Data quality 

After the completion of mean values for ICS FAOSTAT fish categories for all the nutrient values of interest. We proceeded to check the individual values. Most of the check were done as "overall dataset" and then by category. The script used to check the nutrients can be found in "QC.R". 

```{r}

source(here::here("final_qc.R"))

```



##### 5.1. Non-transformed components

We checked the components that were not transformed or calculated. This components were also complete or almost complete for all the FCTs, and hence they are good to check the quality of the importing and to confirm that the method (i.e., using R to generate the NCT) is robust and did not introduce any bias. 
...
to validate the values against the original Fisheries Global NCT, 2022
and to check that all datasets were correctly imported.

In addition, we could use the water content to check the quality of the fish description and try to identify other issues, for example, we checked the water content of fish and preparations taht were reported as "dried products" and of `fish_prep == "cured"`, to make sure that the water content of the fishery products were in line with their description. 

There were some fish items that seemed to have a rather high water content for dried products. 
...
From Fisheries Global NCT (Feb., 2022) notes (ICS SUA code 1501): "The
calcium values of JA15-10213 and BR11-296 are relatively high and the
potassium value of BA13-09_0066 is relatively high. These items are
included considering the ISSCAAP code and limited impact on the average
among the large number of records".

Note that "BR11-296" is not reported in 1501 but in 1502 because it was
reported as "frozen". Similar level of variability can be seen in items
in ICS SUA code (1507). Additionally, a more extreme values (within the
category values context) were found for category 1531.

According to the boxplot the fishery items in ICS SUA code 1555 were
higher than other but it corresponded to only two fish entries which
were both dried with very low water content.
...
##### 5.2. Components that were transformed

###### 5.2.1. Fibre

Fibre concentration of fish and other aquatic animals were mostly zero
and/or `NA` when raw whereas preparations and canned were below 1g per
100g. Then, aquatic plants were on average higher than 1g up to 41g per
100g for aquatic plants preparations.

Current mean of FIBTGg =
`fao_fish_fct %>% filter(!is.na(FIBTGg)) %>% pull(FIBTGg) %>% as.numeric() %>% mean(., na.rm = T)`.

Fibre, crude (FIBCg) could not be used to complete missing values in
FIBTGg, thus the "standardised" value were only values from the FIBTGg,
as FIBCg did not added information on any missing (FBGTg).

For raw fishes, `NA` values for fibre could be assume equal to zero, as
they may not be analysed based on the assumption that they will be zero
for that food type.

When we looked at (FAO) SUA fisheries categories fibre mean values, most
of the values were zero, only "Aquatic Plants", "Aquatic Plants Dried"
and "Aquatic Plants Preparations" contained more than 1g of fibre per
100g.

Individual `NA` values (i.e., per fish entry) were not assumed zero, but
they were "ignored" when calculating the category mean.

In the original Fisheries Global NCT,2022 there were some manually added
"zero" as assumed values. These values were not imported in our dataset,
as we decided to not impute values as much as we could.\
...
Of the twelve FCTs, 10 did not report `ALCg`, of the three that did report it,
    the mean value was zero. Hence, we could assume alcohol was zero for fish and products.
    
###### 5.2.3. Carbohydates and other re-calculated variables

Five FCTs/FCDBs did not report CHOAVLDFg (`r fao_fish_fct %>% filter(is.na(CHOAVLDFg)) %>% distinct(source_fct)`). Of those, two reported CHOCDFg instead (`r fao_fish_fct %>% filter(is.na(CHOAVLDFg), !is.na(CHOCDFg))`), and other two reported CHOAVLg (`rfao_fish_fct %>% filter(is.na(CHOAVLDFg), !is.na(CHOAVLg)) %>% count(source_fct)`). In addition, IN17 did not reported content of carbohydrated and fibre for fish and
    products. 

Then, carbohydrates by difference were calculated using the functions developed and described before.     

Finally, we evaluated the difference in `CHOVALDg_std` values between the
Fisheries Global NCT, 2022 (Feb) and the Fisheries Global NCT, 2022 (Oct), and it is reported in section XX. 
...
Retinol was provided in most of the FCTs, with only 115 entries having a
missing value which were mainly reported in BA13 followed by US19. However, it was particularly problematic for Aquatic mammals (see Plot:
Missing values for retinol by ICS code), for which led to between high
...
Therefore, we used the function `RETOLmcg_Recalculator()` to impute the
missing values.


**Comparison with previous version**

The comparison of the values can be seen in the `final_qc.R` script. From
the previous version (Feb., 2022) to the newest version (Oct., 2022),
41% (39/95) of the retinol values for the SUA fish categories were
exactly the same. Of the other 60% of the values variation occurred from
`r print(retol_max)`mcg to `r print(retol_min)` mcg. However, taking in
consideration that the standard deviation of this food component is
`r sd(as.numeric(fao_fish_fct$RETOLmcg), na.rm = T)`, and that we added over
200 new fisheries entries, variation was expected.

When we looked at the main reasons for data discrepancies were:

**1) New values from NO21**

For example, for Freshwater and Diadromous, Cured (code 1505), four new
values were added, however because the mean was used and one of the
values was high (3300mcg), the new retinol value is considerably higher
than the previous value. We still keep the values, as all the new values
added were with range and similar to other values within the same SUA
fishery category. Similarly, for "Demersal Cured" (code 1518) where
values were 22.65mcg higher due to the influence of NO21 values,
particularly one of the value which was 33mcg. Contrary, for "Demersal,
Canned" (code 1519), the new value from NO21 (2mcg) made the overall
mean of the category to be smaller that the previous version. This
highlight the great influence of high/low values into the mean.

**2) Imputed values**

In absence of retinol values, two approaches were taken in the first
version of the Fisheries dataset (Feb, 2022): 1) Imputing values from
other food/ fisheries items, and 2) re-calculating retinol from other
components (see section 4.4).

Some examples of imputed values from other fish entries and their impact
on final results:

For example (line 2430, code 1530) - There is a manually imputed value
for RETOLmcg (31). The mean value of RETOLmcg with the added imputed
value is 59.5mcg and without is 60mcg. This value was again used in
lines 3040 & 3064, code 15291 & 15301, one value was imputed (RETOLmcg =
31mcg from BA13 - 09_0068) and the difference between the sum of
products reported (99mcg), and the mean value (102mcg) is again small.
Same value in line 3586 (code 1542) with differences by 47vs48mcg (sum
of products vs mean w/o value).

Similarly, lines 2513-2516 (code 1532), three values were imputed from
other fishes, resulting in a difference between the sum of product from
17mcg to 19mcg (when using the mean), which is bigger than the
difference of removing the imputed values form the mean (19mcg vs
19.mcg).

-   Lines 3095, 3096 & 3101 (code 15312), there are three imputed
    values. The sum of products is 6mcg (while the mean is 7mcg), and
    the mean without the imputed is 6mcg.

-   Line 3735 (code 1544), imputed value from unknown origin. Sum of
    products = 36.1mcg, average 49.3 and 52mcg.

-   Lines 3821:2813 (code 1545), mean retinol were 18.0 , 19.9 and
    20.0mcg respectively.

-   Lines 4194, 4200, 4207 & 4227 (code 1553), mean values were 12, 12,
    and 13mcg.

-   Lines 4738:4766 (codes 1580, 1582 & 1583) --\> Odd values and odd
    calculations for "Aquatic mammals".

Hence, we could assume no difference higher than the one occurring by
using sum of products vs mean. While by manually imputing values, we are
adding more uncertainty, we are masking data gaps and we are
hampering the transparency and reproducibility of the data.

Note: All the examples outlined above were manually checked (LSdlR).

In some cases, for example (code 1582), the final value is a combination
of imputed values which made the final average different between the two
version of the dataset. However, the difference between the two values
are small compared with the standard deviation of the results. This
highlight that the added value to imputing values may not be enough to
justify it when compared with the limitations of imputing values.

**3) Typos**

```{r}

results <- mean(as.numeric(fao_fish$`RETOL(µg)`[fao_fish$`ICS FAOSTAT
SUA Current Code`== "1541"][c(1:154)]), na.rm = T)

```


Due to the nature of the manually processed data in excel, some human
error were introduced. For example, for the SUA fishery category (code
1541), the value shown in the "mean" of the Feb., 2022 is 19mcg however
only took in count 63 entries of the 154 values included in that
category. When taking into account all the fish entries we see that the
mean is `r print(results)` in comparison with
the
`r mean(recalculated_results_table$RETOLmcg[recalculated_results_table$ICS.FAOSTAT.SUA.Current.Code == "1541"])`.

###### 5.2.4. Beta-Carotene equivalents

This is a calculated variable, hence when possible (i.e, when the
carotenoids were reported), re-calculation was done. This ensure that we
avoid propagation of error across data sources. However, this was not
always possible hence some imputation technique were used as described
in section 4.4.

Around 9 SUA fishery categories has high amount of missing values (over
80%), see in the missing values plots reported in `QC.R`.

Some constrains of using the imputation technique was that there were
some cases where retinol was greater that Vitamin RAE, this affected to
19 fisheries entries, but were only 7 unique fisheries entries. We then
transformed negative values to zero, assuming that if Vitamin A was
smaller than the retinol value, then no carotene would be presented.
...
In absence of carotenoids, and beta-carotene equivalent values, similar
approach was used as for retinol to complete values. 1) Imputing values
from other food/ fisheries items, and 2) re-calculating beta-carotene
from other components (see section 4.4). However, the application of
this approach was more often due to higher number of missing values, and
less consistent across the dataset. Both in term of reporting the source
and/or reason of the values, and in the prioritasion of the sources
(i.e., using `CARTEBmcg` over re-calculating from `VITA_RAEmcg` &
`RETOLmcg`).

For example, in line 51 & 53 (code 1501), two values were calculated
from `VITA_RAEmcg` & `RETOLmcg`, even when `CARTEBmcg` was provided. The
values resulted are extremely high (by three order of magnitude higher
that the rest of the values in that category). Hence, if excluding those
two values the `CARTBEQmcg standardised` would change the value from
23mcg to 2.6mcg.

Similarly, in line 755 & 757 (code 15010), again to extremely high
values were responsible of the higher mean compared with the mean in our
results. Additionally, contrasted the high number of zero values that
seemed assumed for the same category, which again influenced the mean in
contrast with the relatively higher number of `NA` included in the Oct.,
2022 dataset.

As shown for retinol values used in code 1582, the combination of the
imputed values led to different results in both dataset versions.
However, the quality and source of the imputed values should be
reviewed. Two of the imputed values were quite high (115.6), and which
are in absence a repetition of the same value while the rest of the
values are 0. Hence, the mean value is quite high.

The values of retinol and beta-carotene eq. should be looked into with
care as those are source values of the calculation of Vitamin A and
Vitamin A (REA). Hence, inaccuracies and inconsistencies would be
propagated to the other two mean components.

###### 5.2.5. Vitamin A, retinol activity eq.

After all the calculations and imputations in retinol and beta-carotene
equivalents, only differences reminded in for those ICS SUA catergories
where major discrepancies reminded due to the issues explained above.

###### 5.2.6. Vitamin A, retinol eq.

This variable was reported in X datasets, and it was re-caluclated using
the function `VITAmcg_std_creator()` into the vitamin A (retinol eq.)
standardised variable (`VITAmcg_std`).

In US19, it was reported as vitamin A (IU). When retinol and carotenoids
were available, the VITAmcg was calculated using those values. However,
it was not available for around 800 food entries, hence an assumption of
"all vitamin A was retinol" was used and it was calculated using the
conversion factor (0.3) proposed in the FAO / INFOODS Guidelines
Guidelines for Converting Units, Denominators and Expressions Version
1.0 [@fao/units] for over 400 food entries.



##### 5.3. Nutrients for extension
...
great influence in the final mean value
`r mean(as.numeric(fao_fish_fct$NIAmg_std[fao_fish_fct$ics_faostat_sua_english_description == "Other pelagic fish, cured"]))` mg/100g(range:15-45mg/100g). For instance, when we excluded
values that were higher than 40mg/100g, the mean of that SUA category
was `r mean(as.numeric(fao_fish_fct$NIAmg_std[fao_fish_fct$ics_faostat_sua_english_description == "Other pelagic fish, cured" & fao_fish_fct$NIAmg_std <40]))`mg/100g instead. This could be
...
The rest of the SUA categories remained more or less within the same range of values.

###### 5.2.2. Copper

Similarly for Cu, we identified food items that had the highest values:
Octopus from IN17 (fdc_id = R003) which seemed very high, even when
compared with other octopuses values. 
...
line with the water content of that item. However, the Cu value of this fish item is influencing heavily the the very high mean for the category "Cephalopods, cured", (mean
=
`r mean(as.numeric(fao_fish_fct$CUmg[fao_fish_fct$ics_faostat_sua_english_description == "Cephalopods, cured"]))`)

In the future, we may decide to exclude it from the final mean, which would yield more "sensible" mean
value for the category (mean =
...
Moreover, in the future, in order to minimise the influence of one value, more values should be sought for each category.
...
fao_fish_fct %>% select(FAT_g_standardised, F22D6N3g,
...
###### 4.2.7. Water content, protein, Ca, and Fe (non-calculated)
...
###### 4.2.7. DHA and EPA concentration (non-calculated)
...
those SUA category were very broad, and hence the composition was
